...
from .decorators import login_required_admin,login_required_citoyen,login_required_chef
...
def admin(request):
    if  CustomUser.is_authenticated and CustomUser.is_admin:
     return render(request, 'admin.html',{'session': request.session})
#la page technicine 
...
def technicien(request):
   ...
   if  CustomUser.is_authenticated and CustomUser.is_technicien: 
     ...
     return render(request, 'technicien.html',{'i':i})
...
def directeur(request):
    ...
    if  CustomUser.is_authenticated and CustomUser.is_directeur:
     return render(request, 'directeur.html',{'i':i,'session': request.session,'u':u})
#la page de citoyen    
def citoyen(request):
    if  CustomUser.is_authenticated and CustomUser.is_citoyen:
     i=interven.objects.filter(citoyen=request.user)
     
     return render(request, 'citoyen.html',{'session': request.session,'i':i})
...
def create_intervention(request):
    if request.user.is_authenticated and request.user.is_citoyen:
        if request.method == 'POST':
            ...
            c = interven(description=des, date_debut=date_debut, date_fin=date_fin, date_creation=date_creation, citoyen=citoyen)
            c.save()
...
def modify_intervention(request, intervention_id):
    ...
    technicians = CustomUser.objects.filter(is_technicien=True)
    ...
    if request.method == 'POST':
        ...
        # Assign the technician ID to the intervention
    ...
    return render(request, 'modify_intervention.html', {'intervention': intervention, 'technicians': technicians})
def activer(request):
    if request.POST :
        if request.method == 'POST':
         objet_id = request.POST.get('id')
