...
const ConversationMessagesdir = () => {
  ...
  const handleMessageSubmit = () => {
    if (ws1 && ws1.readyState === WebSocket.OPEN && content.trim() !== '') {
      ...
      console.log("dataaaaaaaaa",messageData);
  ...
  <div
    className={`chat-message ${message.sender.id == userId ? 'right' : 'left'} ${visibleMessageId === index ? 'show-details' : ''} pb-4`}
    key={index}
  ...
  >
    ...
    <div className={`flex-shrink-1 rounded py-2 px-3 mr-3 ${message.sender == userId ? 'bg-blue' : 'bg-gray'}`}>
      <div className="font-weight-bold mb-1">Sender: {message.message.sender.username }</div>
      Content: {message.contenu}
                    ...
                    <form className="mt-3" onSubmit={(e) => { e.preventDefault(); handleMessageSubmit(typeMessage); }}>
